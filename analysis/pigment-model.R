...
#p2pdf('mean-predictions-nolabs.pdf', p.mus.nolabs, scale = 2.5, y.plots = 1.25)
...
#p2pdf('mean-predictions.pdf', p.mus, scale = 2.5, y.plots = 1.25)

# variance
p.s2.k <- ggplot() +
  facet_grid(pigment ~ lake, scales = 'free', labeller = label_parsed) +
  geom_ribbon(aes(year.scam, ymin = lwr.s2, ymax = upr.s2),
              filter(pred, lake == 'Kenosee'), alpha = 0.3) +
  geom_line(aes(year.scam, s2, group = segm.s2),
            filter(pred, lake == 'Kenosee', signif.s2),
            color = '#5E8BDE', lwd = 2) +
  geom_line(aes(year.scam, s2), filter(pred, lake == 'Kenosee')) +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = NULL,
...
p.s2.wb <- ggplot() +
  ...
  geom_line(aes(year.scam, s2, group = segm.s2),
            ...
            color = '#5E8BDE', lwd = 2) +
  geom_line(aes(year.scam, s2), filter(pred, lake == 'White~Bear')) +
  scale_y_continuous(limits = c(0, NA)) +
  labs(x = NULL, y = NULL)

# full figure
p.full <- plot_grid(plot_grid(p.mu.k + xlab(NULL), p.mu.wb, NULL,
                              p.s2.k, p.s2.wb,
                              nrow = 1,
                    ...
                    nrow = 2,
                    rel_heights = c(0.95, 0.05))
#p2pdf('mean-variance-predictions.pdf', p.full, scale = 2.5, x.plots = 4/3)
